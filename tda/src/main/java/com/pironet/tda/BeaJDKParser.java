...
public class BeaJDKParser extends AbstractDumpParser {
    private static final Logger LOGGER = LoggerFactory.getLogger(BeaJDKParser.class);
            ...
            try {
                ...
                while (getBis().ready() && !finished) {
                    ...
                    if (locked) {  // Are we outside of a thread dump ?
                        ...
                        if (LOGGER.isDebugEnabled() && line.indexOf("-- end of trace") >= 0) {
    ...
    protected String[] getThreadTokens(String name) {
        ...
        LOGGER.debug("thread matching:" + m.matches());
        for (int iLoop = 1; iLoop < m.groupCount(); iLoop++) {
            LOGGER.debug("iLoop:{}, group:{}", iLoop, m.group(iLoop));
        }
