...
public class HistogramTableModel extends AbstractTableModel {
    private static final Logger LOGGER = LoggerFactory.getLogger(HistogramTableModel.class);
        ...
        if (((value) == null || value.equals("")) && isShowHotspotClasses()) {
            ...
            for (int i = 0; i < elements.size(); i++) {
                if (isIgnoreCase()) {
                    ...
                    assert value != null;
                    if (isNotHotspotClass(((Entry) elements.get(i)).className) && ("".equals(value) || (((Entry) elements.get(i)).className.contains(value)))) {
                        filteredElements.add(elements.get(i));
                    }
                }
            }
    ...
    private boolean isNotHotspotClass(String className) {
        if (LOGGER.isTraceEnabled()) {
            LOGGER.trace("className: {} eval={}", className, (!isShowHotspotClasses() && className.startsWith("<")));
        }
