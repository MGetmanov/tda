...
public class MBeanDumper {
    private static final Logger LOGGER = LoggerFactory.getLogger(MBeanDumper.class);
    ...
    public MBeanDumper(MBeanServerConnection server) throws IOException {
        ...
        try {
            ...
            InternalError ie = new InternalError(e.getMessage(), e);
            throw ie;
        }
        parseMBeanInfo();
    }
    ...
    private void setDumpPrefix() {
        try {
            RuntimeMXBean rmbean = ManagementFactory.newPlatformMXBeanProxy(server,
    ...
    void setMBeanServerConnection(MBeanServerConnection mbs) {
        this.server = mbs;
        try {
            this.tmbean = ManagementFactory.newPlatformMXBeanProxy(server, ManagementFactory.THREAD_MXBEAN_NAME, ThreadMXBean.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
